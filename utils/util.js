                  ...
                  getApp().login({
                    ...
                    success: function(token) {
                      ...
                      if (page[page.length - 1 >= 0 ? page.length - 1 : 0]) {
                        page[page.length - 1 >= 0 ? page.length - 1 : 0].onLoad(
                          page[page.length - 1 >= 0 ? page.length - 1 : 0]
                            .options
                        );
                      }
                    },
                    ...
                    error: function() {
                      ...
                      console.log("------------------------------------");
                      if (page[page.length - 1 >= 0 ? page.length - 1 : 0]) {
                        page[page.length - 1 >= 0 ? page.length - 1 : 0].error(
                          data_core
                        );
...
function wxlogin(that) {
  ...
  wx.login({
    success: function(res) {
      if (res.code) {
        var url =
          config("baseApiUrl") + "?g=api&m=WeApp&a=login&code=" + res.code;
        ajax({
          url: url,
          method: "GET",
          success: function(data) {
...
function toast(self, mess) {
  ...
  setTimeout(function() {
    self.setData({ btn_order_done: false });
    self.setData({ toast: { toast: 0, mess: "" } });
  }, 2000);
}
...
function succNetCon(self) {
  ...
  if (
    self.data.notNetCon &&
    self.data.notNetCon.error &&
    self.data.notNetCon.show
  )
    loadding(self);
  self.setData({
    notNetCon: {
